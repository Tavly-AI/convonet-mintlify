...
description: "Build a Tavly chat agent for text conversations in the dashboard or API, convert a voice agent to chat, and deploy via web widget, SMS, or your own app."
---
...
<Note>
  Tavly has no built-in integrations with third-party chat platforms such as WhatsApp or Messenger. Beyond the [website widget](/deploy/chat-widget) and [SMS](/deploy/enable-sms), you connect your own channel through the [chat API](/api-references/create-chat).
</Note>
...
To create chat agents programmatically, call [Create Chat Agent](/api-references/create-chat-agent) with a Tavly LLM or conversation flow response engine.
...
* **Website widget** — embed a single `<script>` tag on your site for text (and optional voice) chat with no backend required. See [Tavly Website Widget](/deploy/chat-widget).
...
<AccordionGroup>
  ...
  <Accordion title="How does a chat session end?">
    Three ways: the agent ends it (an end-chat node or function), you end it with the [End chat API](/api-references/end-chat), or the user goes silent past the inactivity timeout and Tavly closes it automatically.
  </Accordion>
