...
---
> Tune Tavly denoising modes, interruption sensitivity, and background noise handling so your voice agent stays accurate in loud or noisy environments.
...
This mode applies background voice cancellation (BVC) to reduce background speech and noise in the caller's audio. It may also suppress or distort the caller's speech, so test it with audio representative of your use case before enabling it broadly. Call recordings preserve the raw caller audio from before BVC, so background noise and speech may still be audible in recordings even when filtered out of the audio processed by the agent.
