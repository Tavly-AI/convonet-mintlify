...
---
> Configure TTS fallback voices so Tavly agents keep speaking during a TTS provider outage — required when using custom or third-party provider voices.

<Note>
  If you are using a [platform voice](/build/speech/platform-voices) or a standard Tavly voice from the voice library, fallback is handled automatically — no additional setup is required. The steps below apply only if you are using a custom or third-party provider voice.
</Note>
...
You can set up your own fallback plan, so that it sounds similar, and the user would not notice the change.
