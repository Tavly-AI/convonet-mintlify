...
<Note>
  If you are using a [platform voice](/build/platform-voices) or a standard Tavly voice from the voice library, fallback is handled automatically — no additional setup is required. The steps below apply only if you are using a custom or third-party provider voice.
</Note>
