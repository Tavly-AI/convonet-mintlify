...
* Google Docs, Sheets, and Drive files, synced through Google Drive knowledge base sync {/* broken-link: /integrations/google-drive-knowledge-base target page does not exist in this repo */}
* Microsoft OneDrive files, synced through OneDrive knowledge base sync {/* broken-link: /integrations/microsoft-onedrive-knowledge-base target page does not exist in this repo */}
* Notion pages, synced through Notion knowledge base sync {/* broken-link: /integrations/notion-knowledge-base target page does not exist in this repo */}
...
<Steps>
  ...
  <Step title="Create Knowledge Base Items">
    ...
    4. **Google Drive**: Sync Docs, Sheets, and files from a connected Drive
       * Connect Google Drive {/* broken-link: /integrations/google-drive target page does not exist in this repo --> first; the Add menu then lists your connected accounts. See Google Drive knowledge base sync <!-- broken-link: /integrations/google-drive-knowledge-base target page does not exist in this repo */} for picking files and how refresh works.
    ...
    5. **Microsoft OneDrive**: Sync files from a connected personal or work OneDrive
       * Connect OneDrive {/* broken-link: /integrations/microsoft-onedrive target page does not exist in this repo --> first; the Add menu then lists your connected accounts. See OneDrive knowledge base sync <!-- broken-link: /integrations/microsoft-onedrive-knowledge-base target page does not exist in this repo */} for picking files, supported formats, and how refresh works.
    ...
    6. **Notion**: Sync pages from a connected Notion workspace
       * Connect Notion {/* broken-link: /integrations/notion target page does not exist in this repo --> first and share pages with the connection in Notion; the Add menu then lists your connected accounts. See Notion knowledge base sync <!-- broken-link: /integrations/notion-knowledge-base target page does not exist in this repo */} for picking pages, what syncs from a page, and how refresh works.
...
Your agent will now use this information when responding to queries related to the added content. Retrieved knowledge base content will be appended to the LLM prompt under the header **## Related Knowledge Base Contexts**. Because these chunks are part of the prompt, they count toward the prompt token surcharge {/* broken-link: /accounts/billing-exceptions#rule-2-llm-price-scaling-for--4000-token-prompt-length target page does not exist in this repo */}. Retrieving more chunks raises the token count.

## Check what the agent retrieved

To see exactly which chunks the agent pulled on a given turn, open the call in Call History {/* broken-link: /features/session-history target page does not exist in this repo */} and go to its transcript. Any agent message that used the knowledge base shows a **Knowledge Base Retrieval** link above it.
