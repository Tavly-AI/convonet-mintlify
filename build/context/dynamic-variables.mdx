---
title : "Dynamic variables : personalize Tavly agent calls"
---
> Inject per-call data into Tavly agents using `{{variable_name}}` syntax for personalized greetings, context-aware responses, and dynamic call routing.
...
<Steps>
  ...
  <Step title="Test your dynamic variables">
    Before deploying, test your dynamic variables using the web interface. You can also set test values for each variable in a simulation test case {/* broken-link: /test/llm-simulation-testing target page does not exist in this repo */}, so automated test runs resolve placeholders the same way real calls do.
  ...
  <Step title="Implement in production">
    ...
    When using the Create Phone Call {/* broken-link: /api-references/create-phone-call target page does not exist in this repo */} API, set your variables in the
    `tavly` field. Note that all values must be strings:
    ...
    You can supply dynamic variables in the Inbound Call Webhook {/* broken-link: /features/inbound-call-webhook target page does not exist in this repo */}. More details are at the linked doc.
  </Step>
</Steps>

> **Important:** All values in `tavly_llm_dynamic_variables` must be strings. Numbers, booleans, or other data types are
> not supported. A number is stored as `"42"` and a boolean as `"true"`. In a [Code Tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript) or [code node](/build/setup/conversation-flow/add-nodes/code-node-run-javascript), cast them before use — for example `Number(dv.order_count)`, `dv.is_member === "true"`, or `JSON.parse(dv.items)`.

<Note>The spaces around the variable name will be trimmed when evaluating the variable.</Note>

## Default System Variables

Tavly automatically provides these system variables - no configuration required:
...
<Note>
  The timezone in `{{current_time_[timezone]}}`, `{{current_hour_[timezone]}}`, and `{{current_calendar_[timezone]}}` is part of the variable name, so it can't be swapped by overriding a `timezone` variable directly. To set the timezone per call, pass it as a dynamic variable and use [nested variables](#nested-variables): write `{{current_time_{{my_timezone}} }}` in your prompt and pass `my_timezone` (for example `"America/Los_Angeles"`) at call start.
</Note>
...
These variables exist only after the session ends, so only a post-call or post-chat function on the [agent's workflow](/build/agent-workflow) can read them: `{{call_summary}}`, `{{call_successful}}`, `{{user_sentiment}}`, `{{disconnection_reason}}`, `{{call_status}}`, and one variable per custom Post Call Extraction {/* broken-link: /features/post-call-analysis-overview target page does not exist in this repo */} field, named after the field. On chat agents the first two are `{{chat_summary}}` and `{{chat_successful}}`, and the status is `{{chat_status}}`.

### Contact Variables

When a phone call matches a contact {/* broken-link: /features/contacts target page does not exist in this repo --> by phone number, that contact's fields are passed in as variables automatically. You get `{{first_name}}`, `{{last_name}}`, `{{do_not_call}}`, and one variable per custom contact field, named after the field. Nothing is passed when no contact matches, so write the prompt to read correctly without them. See contact memory <!-- broken-link: /integrations/build-contact-memory target page does not exist in this repo */} for filling those fields from past conversations.

## Nested Variables

Tavly supports nested variables. You can use the following syntax to create nested variables:
...
* [Community Templates](https://docs.google.com/document/d/1hx6hdTEjAR4y4xXZ7RLMH2byQNVW1ABxC8S4FwvTx_Y/edit?tab=t.0#heading=h.wf5bktkelope): Examples and patterns from the Tavly community
* API Reference {/* broken-link: /api-references/create-phone-call target page does not exist in this repo */}: Full documentation on passing dynamic variables
* Inbound Webhook Guide {/* broken-link: /features/inbound-call-webhook target page does not exist in this repo */}: Setting variables for incoming calls
