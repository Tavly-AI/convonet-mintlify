...
---
> Configure Tavly agents to detect voicemails and IVR systems on outbound calls and choose what to do next — hang up, leave a message, or navigate the menu.

When making outbound calls {/* broken-link: /deploy/outbound-call target page does not exist in this repo */}, your agent may reach a voicemail or an IVR (Interactive Voice Response) menu instead of a person. You can configure the agent to detect these automatically and choose what happens next.
...
To capture keypad input from the person on the other end, see [capture DTMF input](/build/telephony/user-dtmf).
...
* **Leave a message** — wait for the agent's turn to speak, then deliver a message. Supports [dynamic variables](/build/context/dynamic-variables).
...
<Note>
  IVR hangup is separate from [IVR navigation](/build/setup/single-multi-prompt/Press-Digit-IVR), which lets your agent interact with and navigate through IVR menus using DTMF tones.
</Note>
...
<AccordionGroup>
  <Accordion title="Does voicemail detection work on inbound calls?">
    No. Voicemail and IVR detection only run on outbound calls {/* broken-link: /deploy/outbound-call target page does not exist in this repo */}, where your agent dials out and may reach a voicemail or automated menu. On inbound calls a person is calling your agent, so there is no voicemail to detect and these settings have no effect.
  </Accordion>
  ...
  <Accordion title="Will voicemail or IVR detection run for the entire call?">
    No, voicemail and IVR detection will only run for the first 3 minutes of the call. Please contact [support@tavly.com](mailto:support@tavlyai.com) if you need to extend this.
  </Accordion>
  ...
  <Accordion title="How can I tell if a call reached a voicemail or IVR?">
    If voicemail is detected, the call will have a disconnection reason {/* broken-link: /reliability/debug-call-disconnect target page does not exist in this repo */} of `voicemail_reached`. If IVR is detected, the disconnection reason will be `ivr_reached`.
  </Accordion>
