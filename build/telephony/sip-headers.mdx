...
Use custom SIP headers to pass metadata between your systems and Tavly on a call — for example, forwarding an account ID or a caller reference from your [custom telephony](/deploy/custom-telephony) platform so the agent can use it during the conversation.
