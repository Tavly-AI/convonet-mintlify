...
---
> Capture DTMF keypad input from callers in Tavly agents — accept PINs, menu choices, and numeric entries, and configure completion timing and digit counts.
...
To capture voicemail or IVR responses on outbound calls instead, see [handle voicemail and IVR](/build/telephony/handle-voicemail).
...
<AccordionGroup>
  <Accordion title="Which DTMF format does Tavly capture?">
    Tavly captures RFC 2833 DTMF (keypad tones sent as RTP events) by default. If digits aren't being recognized, you can confirm how they were sent by analyzing the call's PCAP file {/* broken-link: /reliability/debug-calls-pcap#extract-and-inspect-dtmf-events target page does not exist in this repo */} in Wireshark.
  </Accordion>
  ...
  <Accordion title="Digits the caller pressed aren't showing up. What should I check?">
    Confirm the agent prompted for keypad input, then inspect the PCAP for a DTMF payload-type mismatch between the SDP and the RTP packets. See debug SIP calls using PCAP files {/* broken-link: /reliability/debug-calls-pcap#extract-and-inspect-dtmf-events target page does not exist in this repo */}.
  </Accordion>
</AccordionGroup>
