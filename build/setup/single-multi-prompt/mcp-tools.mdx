---
title: 'MCP Tools for single/multi-prompt agents'
---

> Connect a Tavly single- or multi-prompt agent to a remote MCP server so it can call the server's tools during a live voice or chat conversation.

Connect your single- or multi-prompt agent to a remote [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server, and the agent can call that server's tools during a live call. Instead of wiring up one [custom function](/build/setup/single-multi-prompt/Custom-Function-Integration) per action, you point the agent at an MCP server once and choose which of its tools the agent can use.
...
If you only need to call your own API endpoint, a [custom function](/build/setup/single-multi-prompt/Custom-Function-Integration) is simpler — there's no MCP server to run. For self-contained logic that doesn't need a server, use a [code tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript).

<Note>
  This page is about connecting your agent **to** a remote MCP server so it can call that server's tools. It's not the same as Tavly's own MCP server {/* broken-link: /get-started/mcp-server target page does not exist in this repo */}, which lets MCP clients like Cursor and Claude Desktop manage your Tavly account.
</Note>

### Example: order lookup over MCP

A retail support team already runs an MCP server that exposes `lookup_order` and `create_ticket` tools to its internal assistants. You connect a Tavly support agent to the same server and enable both tools. When a caller gives an order number, the agent calls `lookup_order` and reads back the status; if the caller reports a problem, it calls `create_ticket` — with no separate custom function for either action.

## How it works

You attach the MCP server to the agent, and each tool you enable becomes a tool the agent can call whenever your prompt and the tool's description tell it to — the same way [custom functions](/build/setup/single-multi-prompt/Custom-Function-Integration) and [prebuilt functions](/build/setup/single-multi-prompt/Function-Calling-Overview) work. Tavly connects to the server over Streamable HTTP, lists the server's tools, and calls a tool with arguments the LLM fills in from the conversation. The tool's response is handed back to the agent, which can speak it, act on it, or save parts of it as [dynamic variables](/build/context/dynamic-variables).
...
<Steps>
  ...
  <Step title="Set request headers (optional)">
    Under **Headers**, add the key/value pairs Tavly sends with the MCP connection request — most often an auth header such as `Authorization: Bearer <token>`. Click **New key value pair** for each header. Values can include [dynamic variables](/build/context/dynamic-variables), so you can pass a per-call token as `{{api_key}}`.
  </Step>

  <Step title="Set query parameters (optional)">
    Under **Query Parameters**, add key/value pairs Tavly appends to the server URL as a query string. Use this for servers that authenticate or route by query parameter. Values can include [dynamic variables](/build/context/dynamic-variables) too.
  </Step>
  ...
  <Step title="Add a tool">
    On the saved server, click **Add Tools**. In **Tool Access Scope**, open the **Select Tool** dropdown and pick a tool — Tavly connects to the server and lists its tools, each with its name and description.
  ...
  <Step title="Set response variables (optional)">
    In **Store Fields as Variables**, extract values from the tool's response and save them as [dynamic variables](/build/context/dynamic-variables) to use later in the call. Each row maps a variable name to the path of a field in the response, using dot notation with array indexing where needed — for example `user.name` or `data.items[0].id`. This works only when the tool returns a JSON object.
...
<AccordionGroup>
  <Accordion title="What's the difference between an MCP tool and a custom function?">
    Both let the agent take an action mid-call. A [custom function](/build/setup/single-multi-prompt/Custom-Function-Integration) calls one HTTP endpoint you host. An MCP tool connects to a remote MCP server that can expose many tools through a single connection, and Tavly discovers those tools from the server. Use MCP when the tools already live behind an MCP server; use a custom function to call your own API directly.
  </Accordion>
  ...
  <Accordion title="What transport does Tavly use to reach my MCP server?">
    Tavly connects over Streamable HTTP. The server URL must be publicly reachable over `http` or `https` — Tavly rejects any other protocol and blocks URLs that point to `localhost`, private network ranges, or cloud metadata endpoints. Pass any auth the server needs through the request headers or query parameters.
  </Accordion>

  <Accordion title="My MCP server requires OAuth. Can Tavly handle the OAuth flow?">
    No. Tavly authenticates to your MCP server only through the request headers and query parameters you configure — it doesn't run an interactive OAuth authorization flow. If your server requires OAuth, obtain the access token outside of Tavly and pass it in a header such as `Authorization: Bearer <token>`. For short-lived tokens, use a [dynamic variable](/build/context/dynamic-variables) like `{{access_token}}` in the header value and pass a fresh token with each call.
  </Accordion>

  <Accordion title="Can I restrict my MCP server to requests from Tavly?">
    Yes. Tavly's outbound requests come from the IP address `100.20.5.228`, the same address used for [custom functions](/build/setup/single-multi-prompt/Custom-Function-Integration) and webhooks {/* broken-link: /features/webhook-overview target page does not exist in this repo */}. Allowlist it at your firewall, and add a secret in the request headers or query parameters as a second check.
  </Accordion>

  <Accordion title="Can I use dynamic variables in the MCP server URL?">
    Yes. The URL accepts [dynamic variables](/build/context/dynamic-variables) the same way headers and query parameters do — for example `https://mcp.example.com/{{tenant_id}}`. Tavly fills them in when it connects. Every variable in the URL must resolve at call time; if one is still unset, the connection fails with an error instead of calling an incomplete URL.
  </Accordion>

  <Accordion title="Does Tavly retry a failed tool call?">
  ...
  <Accordion title="Is there a limit on the tool response?">
    Yes. Tavly uses the tool's text response and caps it at 15,000 characters by default before handing it to the LLM, so return only what the agent needs. Contact support if you need a higher limit.
  </Accordion>

  <Accordion title="Can I use MCP tools in a conversation flow agent instead?">
    Yes. Conversation flow agents call MCP tools through a dedicated [MCP node](/build/setup/conversation-flow/add-nodes/mcp-node-in-conversation-flow) rather than adding them to the response engine. The server configuration (URL, headers, query params) is the same; the difference is where the tool call fits in the flow.
  </Accordion>
</AccordionGroup>
