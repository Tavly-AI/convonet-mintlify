...
<Note>
  This page is about connecting your agent **to** a remote MCP server so it can call that server's tools. It's not the same as [Tavly's own MCP server](/get-started/mcp-server), which lets MCP clients like Cursor and Claude Desktop manage your Tavly account.
</Note>
...
<Steps>
  ...
  <Step title="Set request headers (optional)">
    Under **Headers**, add the key/value pairs Tavly sends with the MCP connection request — most often an auth header such as `Authorization: Bearer <token>`. Click **New key value pair** for each header. Values can include [dynamic variables](/build/dynamic-variables), so you can pass a per-call token as `{{api_key}}`.
  </Step>

  <Step title="Set query parameters (optional)">
    Under **Query Parameters**, add key/value pairs Tavly appends to the server URL as a query string. Use this for servers that authenticate or route by query parameter. Values can include [dynamic variables](/build/dynamic-variables) too.
  </Step>
...
<AccordionGroup>
  <Accordion title="What's the difference between an MCP tool and a custom function?">
    Both let the agent take an action mid-call. A [custom function](/build/single-multi-prompt/custom-function) calls one HTTP endpoint you host. An MCP tool connects to a remote MCP server that can expose many tools through a single connection, and Tavly discovers those tools from the server. Use MCP when the tools already live behind an MCP server; use a custom function to call your own API directly.
  </Accordion>
  ...
  <Accordion title="My MCP server requires OAuth. Can Tavly handle the OAuth flow?">
    No. Tavly authenticates to your MCP server only through the request headers and query parameters you configure — it doesn't run an interactive OAuth authorization flow. If your server requires OAuth, obtain the access token outside of Tavly and pass it in a header such as `Authorization: Bearer <token>`. For short-lived tokens, use a [dynamic variable](/build/dynamic-variables) like `{{access_token}}` in the header value and pass a fresh token with each call.
  </Accordion>

  <Accordion title="Can I restrict my MCP server to requests from Tavly?">
    Yes. Tavly's outbound requests come from the IP address `100.20.5.228`, the same address used for [custom functions](/build/single-multi-prompt/custom-function) and [webhooks](/features/webhook-overview). Allowlist it at your firewall, and add a secret in the request headers or query parameters as a second check.
  </Accordion>

  <Accordion title="Can I use dynamic variables in the MCP server URL?">
    Yes. The URL accepts [dynamic variables](/build/dynamic-variables) the same way headers and query parameters do — for example `https://mcp.example.com/{{tenant_id}}`. Tavly fills them in when it connects. Every variable in the URL must resolve at call time; if one is still unset, the connection fails with an error instead of calling an incomplete URL.
  </Accordion>
