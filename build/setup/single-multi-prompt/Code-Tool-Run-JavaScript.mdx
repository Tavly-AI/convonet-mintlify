---
title: 'Code Tool: Run JavaScript in Tavly AI Agents'
---

> Add a Code tool to a single- or multi-prompt Tavly AI agent to run JavaScript, read dynamic variables and metadata, call HTTP APIs, and store results.

Code Tool lets a single- or multi-prompt agent run JavaScript in Tavly AI's
...
If you use a conversation flow agent, see [Code node](/build/setup/conversation-flow/add-nodes/code-node-run-javascript).

## When to use Code Tool

|                       | Code Tool                                           | Custom function                                              |
| --------------------- | --------------------------------------------------- | ------------------------------------------------------------ |
| **Runs**              | JavaScript in Tavly AI's sandbox                      | An HTTP request to your server                               |
...
<Note>
  ...
  calls the tool, use a [custom function](/build/setup/single-multi-prompt/Custom-Function-Integration).
</Note>
...
<Steps>
  ...
  <Step title="Write JavaScript">
    ...
    value. Tavly AI serializes the return value to JSON, sends it to the LLM as
...
Read [dynamic variables](/build/context/dynamic-variables) as properties of `dv`. All
...
[Extract Dynamic Variable tool](/build/setup/single-multi-prompt/extract-dynamic-variables) or return
...
Read metadata passed when you create the phone call {/* broken-link: /api-references/create-phone-call target page does not exist in this repo */}
or chat {/* broken-link: /api-references/create-chat target page does not exist in this repo */}.
...
Code Tool `fetch()` requests originate from `35.166.138.221`. If your public
...
path doesn't exist or resolves to `null`, Tavly AI skips that variable without
...
<AccordionGroup>
  ...
  <Accordion title="What happens if the code times out or throws an error?">
    Tavly AI marks the execution as failed, sends the error to the LLM, doesn't extract response variables, and doesn't retry automatically. Use `try/catch` for expected errors and return a structured fallback such as `{ "ok": false, "error": "lookup failed" }`.
  </Accordion>
  ...
  <Accordion title="Is there a limit on the result size?">
    Tavly AI sends up to 15,000 characters to the agent by default. Response-variable extraction uses the full return value before that result is capped.
  </Accordion>
</AccordionGroup>
