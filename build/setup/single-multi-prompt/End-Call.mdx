...
* **Add the End Call tool programmatically**: Include a tool with `type: end_call` in `general_tools` when you [create](/api-references/create-retell-llm) or [update](/api-references/update-retell-llm) a Tavly AI LLM. This is equivalent to adding the tool in the dashboard.
* **Custom LLM integrations**: If you bring your own LLM over WebSocket, set `end_call: true` in your response and Tavly AI ends the call after the content is fully spoken. See the [LLM WebSocket reference](/api-references/llm-websocket).
