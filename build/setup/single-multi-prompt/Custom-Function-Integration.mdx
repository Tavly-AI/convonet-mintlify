---
title: 'Custom Function Integration'
---

# Integrate any system with custom function

> Add a custom function to a Tavly AI single- or multi-prompt agent to call your API mid-call. Configure URL, method, headers, parameters, and responses.

A custom function lets your agent call your own API during a live call, then use the response to keep the conversation going. Use it to look up an order, check an account, create a ticket, or trigger any action in your backend that the agent can't do on its own. For common CRM, helpdesk, and calendar actions, [integration tools](/build/setup/single-multi-prompt/Integration-Tools) call the provider directly with no server of your own to host.
...
If the logic is self-contained and doesn't need your server — a calculation, reading a [dynamic variable](/build/context/dynamic-variables), or a request to a public API — a [code tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript) runs it in a sandbox with no endpoint to host. For common built-in actions like transferring or ending a call, use the [prebuilt functions](/build/setup/single-multi-prompt/Function-Calling-Overview) instead.
...
The agent calls a custom function whenever your prompt and the function's description tell it to. When it does, Tavly AI sends a request to your URL with the function's arguments and waits for your response before continuing.
...
<Steps>
  ...
  <Step title="Set response variables (optional)">
    In the **Store Fields as Variables** section, extract values from the JSON response and save them as [dynamic variables](/build/context/dynamic-variables) to use later in the conversation.
...
* Headers
  * `X-Retell-Signature`: an HMAC-SHA256 signature of the request body, used to verify the request came from Tavly AI. See [Verify the request is from Tavly AI](#verify-the-request-is-from-tavly-ai) below.
...
To confirm a request came from Tavly AI, verify the `X-Retell-Signature` header against the raw request body using your Tavly AI API key. For GET and DELETE requests the body is empty, so pass an empty string to the verify function.
