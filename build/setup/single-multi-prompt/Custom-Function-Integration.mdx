...
description: "Add a custom function to a Tavly single- or multi-prompt agent to call your API mid-call. Configure URL, method, headers, parameters, and response mapping."
---
...
<Steps>
  ...
  <Step title="Set query parameters (optional)">
    Define query parameters as key/value pairs that Tavly AI appends to the endpoint URL. Values are applied directly and can include [dynamic variables](/build/dynamic-variables). Query parameters are never filled in by the LLM — for LLM-supplied values, use the request body parameters below.
  </Step>
