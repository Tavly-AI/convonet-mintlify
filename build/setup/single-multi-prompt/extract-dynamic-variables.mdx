...
> Add the Extract Dynamic Variable tool to a Tavly AI single- or multi-prompt agent to capture values from a call as typed dynamic variables.

The Extract Dynamic Variable tool captures values from the conversation while a
call is in progress and stores them as [dynamic variables](/build/context/dynamic-variables)
...
If you use a conversation flow agent, see [Extract Dynamic Variable node](/build/setup/conversation-flow/add-nodes/extract-dynamic-variable-node).
...
| Collect structured data only for after the call (analytics, webhook) | Post Call Extraction {/* broken-link: /features/post-call-analysis-overview target page does not exist in this repo */}                                 |
| Run the extraction on a fixed step in a set sequence                 | [Extract Dynamic Variable node](/build/setup/conversation-flow/add-nodes/extract-dynamic-variable-node) (conversation flow) |
| Compute or transform values in code                                  | [Code Tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript)                                             |
...
[conversation flow](/build/setup/conversation-flow/add-nodes/extract-dynamic-variable-node) instead.
...
everywhere else in Tavly AI. A `Number` is stored as `"42"` and a `Boolean` as
`"true"`. In a [Code Tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript), convert them
before use, for example `Number(dv.order_count)`.

## Use the extracted values

Once extracted, a value behaves like any other [dynamic variable](/build/context/dynamic-variables):

* Reference it in prompts and tool fields with `{{variable_name}}` on later
  turns.
* Read it in a [Code Tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript) as `dv.variable_name`.
* Pass it to a [custom function](/build/setup/single-multi-prompt/Custom-Function-Integration) or
  an [agent transfer](/build/setup/single-multi-prompt/agent-transfer-tool).

If the caller never provides a value, the tool doesn't set that variable, and
`{{variable_name}}` stays in its raw form. See
[handling missing variables](/build/context/dynamic-variables#handling-missing-variables)
...
<AccordionGroup>
  ...
  <Accordion title="Is this the same as Post Call Extraction variables?">
    No. This tool captures values *during* the call so the agent can use them in
    later turns. Post Call Extraction {/* broken-link: /features/post-call-analysis-overview target page does not exist in this repo */} extracts data
