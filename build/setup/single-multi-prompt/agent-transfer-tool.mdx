...
> Add the Agent Transfer (agent swap) tool to a Tavly AI single or multi-prompt agent so it can hand the live call to another AI agent mid-conversation.
...
Use [call transfer](/build/setup/single-multi-prompt/Transfer-Call-Tool) instead when the destination is a **human or an external phone number**. Agent Transfer only swaps between Tavly AI agents.

Two targets can't be swapped to: agents backed by a custom LLM {/* broken-link: /integrate-llm/overview target page does not exist in this repo */} and chat-channel agents. Attempting either fails the transfer.

## Agent Transfer vs. call transfer

Compared with routing to another agent through [call transfer](/build/setup/single-multi-prompt/Transfer-Call-Tool), Agent Transfer offers:
...
The swap keeps everything on the same call, with the same `call_id` and one entry in call history {/* broken-link: /features/session-history target page does not exist in this repo */}. The destination agent automatically receives:

* **Full conversation history.** The transcript up to the transfer point, so the new agent has the same context the caller does.
* **Call-level `metadata`.** Set on the original call (for example, via Create Phone Call {/* broken-link: /api-references/create-phone-call target page does not exist in this repo --> or the Inbound Call Webhook <!-- broken-link: /features/inbound-call-webhook target page does not exist in this repo */}) and shared across every agent on the call.
* **Dynamic variables.** Both the `retell_llm_dynamic_variables` passed when the call started and any variables [extracted earlier in the call](/build/setup/single-multi-prompt/extract-dynamic-variables) stay available to the destination agent's prompt.

There is no "pass variables" parameter on the tool. The shared call context above is the mechanism. To have the destination agent act on a specific value, [extract it into a dynamic variable](/build/setup/single-multi-prompt/extract-dynamic-variables) on the source agent and reference that variable (`{{variable_name}}`) in the destination agent's prompt.
...
* **Data storage setting** (`data_storage_setting`): what call data Tavly AI stores.
* **PII redaction** (`pii_config`): the post-call PII scrubbing rules.
* **Denoising mode**: the audio denoising applied to the call.

Everything tied to the active agent switches to the destination agent on each swap: **language, voice, ambient sound, LLM/model, prompt, tools, knowledge bases, boosted keywords, interruption sensitivity, and voice speed.** Two exceptions are opt-in: turn on **keep the same voice** or **keep the current language** to carry the current voice or language into the destination agent instead of using its own. Keeping the same voice also keeps the current ambient sound; otherwise the destination agent's ambient sound applies with its voice.
...
<Steps>
  ...
  <Step title="Configure the transfer">
    ...
    * **Post Call Extraction setting:** which agent's Post Call Extraction {/* broken-link: /features/post-call-analysis target page does not exist in this repo */} runs after the transfer. **Only transferred agent** uses the destination agent's full analysis configuration (its fields, model, and prompts). **Both this agent and transferred agent** keeps the analysis fields from both; where field names collide, the destination agent's win. Including both increases usage cost.
    * **Webhook setting:** which agent's webhook {/* broken-link: /features/webhook-overview target page does not exist in this repo */} receives call events after the transfer: only this (source) agent, only the transferred agent, or both. Defaults to only the source agent.
    * **Keep the same voice / keep the current language** (optional): carry the current voice or language into the destination agent instead of switching to its own. Keeping the same voice also keeps the current ambient sound settings.
...
<AccordionGroup>
  ...
  <Accordion title="Which agents can't I transfer to?">
    Agents backed by a custom LLM {/* broken-link: /integrate-llm/overview target page does not exist in this repo */} and chat-channel agents. Transferring to either fails, and the agent continues on the current call.
  </Accordion>
...
<Note>
  Building in [Conversation Flow](/build/setup/conversation-flow/conversation-flow-overview) instead of a single or multi-prompt agent? Use the [Agent Transfer node](/build/setup/conversation-flow/add-nodes/agent-transfer-node). It has the same behavior, configured as a node.
</Note>
