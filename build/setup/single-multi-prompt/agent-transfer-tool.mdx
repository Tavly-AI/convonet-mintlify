...
Use [call transfer](/build/single-multi-prompt/transfer-call) instead when the destination is a **human or an external phone number**. Agent Transfer only swaps between Tavly AI agents.
...
<Steps>
  ...
  <Step title="Configure the transfer">
    ...
    * **Post Call Extraction setting:** which agent's [Post Call Extraction](/features/post-call-analysis) runs after the transfer. **Only transferred agent** uses the destination agent's full analysis configuration (its fields, model, and prompts). **Both this agent and transferred agent** keeps the analysis fields from both; where field names collide, the destination agent's win. Including both increases usage cost.
