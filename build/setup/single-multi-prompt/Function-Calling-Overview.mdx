...
> Function calling lets Tavly AI single or multi-prompt agents take real actions — transfer calls, end calls, book appointments, send SMS, and call external APIs.
...
3. **Function Execution**: Tavly AI calls your function with the extracted parameters
...
Tavly AI provides ready-to-use functions for common scenarios:

| Function                                                      | Purpose                           | Common Use Cases                             |
| ------------------------------------------------------------- | --------------------------------- | -------------------------------------------- |
| [**End Call**](/build/setup/single-multi-prompt/End-Call)           | Terminate conversation gracefully | After completing tasks, when caller requests |
| [**Transfer Call**](/build/setup/single-multi-prompt/Transfer-Call-Tool) | Route to human agents             | Escalation, department routing               |
| [**Press Digits**](/build/setup/single-multi-prompt/Press-Digit-IVR)    | Send DTMF tones                   | Navigate IVR systems, enter codes            |
| [**Send SMS**](/build/setup/single-multi-prompt/Send-SMS)           | Send text messages                | Confirmations, follow-ups                    |

<Note>
  For appointment scheduling, use the Cal.com integration {/* broken-link: /integrations/cal-com target page does not exist in this repo --> or Calendly integration <!-- broken-link: /integrations/calendly target page does not exist in this repo -->. The built-in **Check Availability** and **Book Calendar** tools are no longer offered here — see the deprecation notice <!-- broken-link: /deprecation-notice/2026/10-31_legacy_calcom_tools target page does not exist in this repo */} for what happens to agents that still have them.
</Note>

### 2. Integration tools

Call a connected provider (HubSpot, Salesforce, Zendesk, Cal.com, and the other supported providers {/* broken-link: /integrations/overview#supported-providers target page does not exist in this repo */}) live during the conversation — look up the caller, book an appointment, create a ticket. Tavly AI handles the provider's API and auth. See [integration tools](/build/setup/single-multi-prompt/Integration-Tools) for setup.
...
[Learn more about custom functions →](/build/setup/single-multi-prompt/Custom-Function-Integration)

### 4. Code tool

Run JavaScript code directly in Tavly AI's sandbox without setting up an external server. Useful for:
...
[Learn more about Code Tool →](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript)
...
* API Reference {/* broken-link: /api-references/create-agent target page does not exist in this repo */}: Technical documentation
* [Best Practices](/build/response/prompt-engineering-guide): Writing effective function prompts, 
