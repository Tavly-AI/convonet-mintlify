...
<Note>
  For appointment scheduling, use the [Cal.com integration](/integrations/cal-com) or [Calendly integration](/integrations/calendly). The built-in **Check Availability** and **Book Calendar** tools are no longer offered here — see the [deprecation notice](/deprecation-notice/2026/10-31_legacy_calcom_tools) for what happens to agents that still have them.
</Note>

### 2. Integration tools

Call a connected provider (HubSpot, Salesforce, Zendesk, Cal.com, and the other [supported providers](/integrations/overview#supported-providers)) live during the conversation — look up the caller, book an appointment, create a ticket. Tavly AI handles the provider's API and auth. See [integration tools](/build/single-multi-prompt/integration-tools) for setup.
