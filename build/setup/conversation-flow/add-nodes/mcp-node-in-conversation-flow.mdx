...
<Steps>
  ...
  <Step title="Set request headers (optional)">
    You can define custom headers to include with the request Tavly sends to your MCP Server.

    Headers and query parameters are the only way Tavly authenticates to your MCP server — most often an auth header such as `Authorization: Bearer <token>`. Tavly doesn't run an interactive OAuth flow; if your server requires OAuth, obtain the access token outside of Tavly and pass it in a header. Values can include [dynamic variables](/build/context/dynamic-variables), so you can pass a per-call token as `{{access_token}}`.

    <Note>To restrict your MCP server to requests from Tavly, allowlist Tavly's outbound IP address: `100.20.5.228`.</Note>
  ...
  <Step title="Set query parameters (optional)">
    You can define query parameters to include in the request URL that Tavly appends to your MCP Server Endpoint.
