...
> Use an end node in a Tavly conversation flow to terminate the call cleanly, optionally with a closing message generated from a prompt or static text.
...
* **Global Node**: Make the end node reachable from anywhere in the flow when its condition is met — for example `User wants to end the call or says goodbye` — so you don't have to wire an edge to it from every node. Read more at [global node](/build/setup/conversation-flow/advanced/global-node).

## End node vs End Call tool

Use an end node when the call should end at a fixed point in the flow. If instead the agent should be able to end the call mid-dialogue whenever it judges the conversation complete, attach an [End Call tool](/build/setup/single-multi-prompt/End-Call) to a [subagent node](/build/setup/conversation-flow/add-nodes/subagent-node).
