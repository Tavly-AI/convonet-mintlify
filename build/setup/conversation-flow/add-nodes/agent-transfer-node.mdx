...
Compared to transferring to another agent using [transfer call](/build/setup/conversation-flow/add-nodes/call-transfer-node), **Agent Transfer** offers significant advantages:
...
For a fuller walkthrough of what the destination agent inherits (transcript, metadata, and dynamic variables) and every configuration option, see [Agent Transfer](/build/setup/single-multi-prompt/agent-transfer-tool).
