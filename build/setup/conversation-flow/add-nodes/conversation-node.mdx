...
> Use Tavly conversation nodes for multi-turn dialogue without tool calls — the default building block for capturing info and guiding callers through a flow.
...
If the agent should also call tools during the same dialogue, use a [subagent node](/build/setup/conversation-flow/add-nodes/subagent-node). If a tool must always run at a fixed point in the flow, use a [function node](/build/setup/conversation-flow/function-node/call-a-function-from-a-conversation-flow-node).
