...
Subagent nodes let a Tavly conversation flow combine dialogue with on-the-fly tool calls — the LLM decides whether and when to use each attached tool.
...
If you only need dialogue without tool calling, use a [conversation node](/build/setup/conversation-flow/add-nodes/conversation-node).
...
Subagent nodes and [function nodes](/build/setup/conversation-flow/function-node/call-a-function-from-a-conversation-flow-node) serve different purposes:
...
| ------------------------ | ------------------------------------------------------------- | ----------------------------------------------------------------- |
| Custom Function          | Make HTTP requests to your external APIs                      | [Custom Function](/build/setup/conversation-flow/function-node/custom-function-in-conversation-flow)       |
| Code Tool                | Run JavaScript code directly without an external server       | [Code Tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript)                 |
| Integration tools        | Act on a connected provider like HubSpot, Zendesk, or Cal.com | [Integration tools](/build/setup/conversation-flow/function-node/integration-tools-in-conversation-flow)   |
| End Call                 | Terminate the call                                            | [End Call](/build/setup/single-multi-prompt/End-Call)                   |
| Transfer Call            | Transfer to a phone number                                    | [Transfer Call](/build/setup/single-multi-prompt/Transfer-Call-Tool)         |
| Transfer Agent           | Transfer to another Tavly agent                              | [Transfer Agent](/build/setup/single-multi-prompt/agent-transfer-tool)       |
| Press Digit              | Send DTMF tones                                               | [Press Digit](/build/setup/single-multi-prompt/Press-Digit-IVR)             |
| Send SMS                 | Send a text message                                           | [Send SMS](/build/setup/single-multi-prompt/Send-SMS)                   |
| Extract Dynamic Variable | Extract variables from the conversation                       | [Extract Dynamic Variable](/build/setup/single-multi-prompt/extract-dynamic-variables) |
| MCP Tool                 | Call tools on your MCP server                                 | [MCP Node](/build/setup/conversation-flow/add-nodes/mcp-node-in-conversation-flow)                     |
...
* After the user finishes speaking, following the [evaluation order](/build/setup/conversation-flow/transition-conditions#evaluation-order).
...
* **Knowledge Base**: Attach node-level knowledge bases to combine topic-specific knowledge with the agent-level knowledge base. Read more at [knowledge base](/build/context/knowledge-base).
* **Global Node**: Make this node reachable from anywhere in the flow when its condition is met. Read more at [global node](/build/setup/conversation-flow/advanced/global-node).
...
* **Fine-tuning examples**: Add example transcripts to improve this node's responses and transition decisions. Read more at [finetune examples](/build/setup/conversation-flow/advanced/finetune-examples).

## Best practices

* **Be explicit in your node instruction.** Tell the agent when each tool should be used.
* **Use function nodes for guaranteed execution.** If a tool must always run at a certain point in the flow, use a [function node](/build/setup/conversation-flow/function-node/call-a-function-from-a-conversation-flow-node) instead.
