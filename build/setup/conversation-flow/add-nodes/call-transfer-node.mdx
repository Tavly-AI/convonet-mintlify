---
title: Call transfer node in conversation flow
---

> Use a call transfer node to hand a Tavly phone call off to a different number. Supports Tavly and imported numbers, with optional pre-transfer messages.

<Warning>
  This node only works during phone calls instead of web calls. It's available for Tavly numbers and imported numbers.
</Warning>
...
<Steps>
  <Step title="Set Up Transfer To Target">
    Set transfer number to be either:

    * a number in e.164 format, or a SIP URI in the format of `sip:username@domain` (e.g. `sip:user@tavlyai.com`).
    * [dynamic variable](/build/context/dynamic-variables) that gets substituted at runtime.
    * (Optional) if your transfer destination is not in e.164 format then you can choose to keep the input as is by choosing raw format. This only applies when you are using custom telephony and does not apply when you are using Tavly Telephony. This can be useful when you want to transfer to internal pseudo numbers.
  ...
  <Step title="Configure Caller ID (Optional)">
    ...
    1. **Tavly Agent's number**: The transfer destination will see the Tavly agent's number.
    ...
    2. **User's Number**: The transfer destination will see the number of the user. Please note that the telephony provider must support caller ID override for this feature to work.
       ...
       * Tavly Twilio numbers support showing user's number on both warm and cold transfer, Tavly Telnyx numbers only support this when using SIP REFER via cold transfer.
