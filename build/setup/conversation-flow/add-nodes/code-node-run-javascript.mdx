---
title: "Code node: run JavaScript in a conversation flow"
---

> Add a Code node to a Tavly AI conversation flow to run JavaScript, read dynamic variables and metadata, call HTTP APIs, and store results.
...
If you use a single- or multi-prompt agent, see [Code Tool](/build/setup/single-multi-prompt/Code-Tool-Run-JavaScript).
...
| --------------------- | --------------------------------------------------- | --------------------------------------------------- |
| **Runs**              | JavaScript in Tavly's sandbox                      | An HTTP request to your server                      |
...
<Warning>
  ...
  a [custom function](/build/setup/conversation-flow/function-node/custom-function-in-conversation-flow) hosted on your
  backend.
</Warning>
...
path doesn't exist or resolves to `null`, Tavly skips that variable without
...
* When it is on, Tavly waits for the code to finish before evaluating the
...
Use [transition conditions](/build/setup/conversation-flow/transition-conditions) that
...
| **Play typing sound**    | Off          | Plays a typing sound while Tavly waits for the result. Applies when **Wait for Result** is on. |
...
<AccordionGroup>
  ...
  <Accordion title="What happens if the code times out or throws an error?">
    Tavly marks the execution as failed, doesn't extract response variables, and doesn't retry automatically. Use `try/catch` for expected errors and return a structured fallback such as `{ "ok": false, "error": "lookup failed" }`. For critical routing, configure a safe else path.
  </Accordion>
  ...
  <Accordion title="Is there a limit on the result size?">
    Tavly sends up to 15,000 characters to the agent by default. Response-variable extraction uses the full return value before that result is capped.
  </Accordion>
</AccordionGroup>
