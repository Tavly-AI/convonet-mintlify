...
> Build structured Tavly voice agents with conversation flow — nodes for dialogue, tools, transfers, code, and transitions for precise call control.
...
* **[Node](/build/setup/conversation-flow/add-nodes/nodes-and-edges)**: The basic unit of conversation flow. Multiple node types are available:
...
* **[Edge](/build/setup/conversation-flow/transition-conditions)**: Connections between nodes that define transition logic:
...
* **Agents**: your main agent (**Main flow**) and any **transfer agents** you open from an [agentic warm transfer](/build/setup/conversation-flow/add-nodes/call-transfer-node).
* **Subflows**: any [subflows](/build/setup/conversation-flow/components) you open for editing.
...
* Via API, set the same `response_engine.conversation_flow_id` on each agent when calling Create Agent {/* broken-link: /api-references/create-agent target page does not exist in this repo --> or Update Agent <!-- broken-link: /api-references/update-agent target page does not exist in this repo */}.
