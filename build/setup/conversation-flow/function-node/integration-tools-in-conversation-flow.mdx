...
description: "Call HubSpot, Salesforce, Zendesk, or Cal.com tools from a Tavly conversation flow function node, and branch outgoing transitions on what comes back."
---
...
See the [integrations overview](/integrations/overview) for the providers Tavly AI connects to, what each one's tools do, and how to connect them. [Subagent nodes](/build/conversation-flow/subagent-node) can use integration tools too, alongside their other tool types. Integration tools can also run outside the conversation, before it starts or after it ends; see [agent workflow](/agent/agent-workflow).
