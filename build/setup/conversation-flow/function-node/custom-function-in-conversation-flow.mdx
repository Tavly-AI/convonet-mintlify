---
title : " Custom function in conversation flow"
description: "Add a custom function to a Tavly AI's conversation flow to call your external API mid-call. Configure method, URL, headers, parameters, and responses."
---
...
Custom functions run inside a [function node](/build/setup/conversation-flow/function-node/call-a-function-from-a-conversation-flow-node). If the logic is self-contained and doesn't need your server, a [code node](/build/setup/conversation-flow/add-nodes/code-node-run-javascript) runs it in a sandbox with no endpoint to host, and for common CRM, helpdesk, and calendar actions, [integration tools](/build/setup/conversation-flow/function-node/integration-tools-in-conversation-flow) call the provider directly.
...
When the function is called, Tavly AI sends a request (POST, GET, PUT, PATCH, or DELETE) to your URL with the function name and arguments. You can add headers and query parameters, and extract values from the response.
...
<Steps>
  ...
  <Step title="Choose the HTTP method">
    Select the method Tavly AI uses to call your endpoint: **GET**, **POST**, **PUT**, **PATCH**, or **DELETE**. It defaults to POST.

    Tavly AI sends a JSON body only for POST, PUT, and PATCH. GET and DELETE carry data as query parameters only.
  </Step>

  <Step title="Add the endpoint URL">
    Enter the URL Tavly AI calls to run the function. It must be a valid, publicly reachable URL. For security, Tavly AI blocks requests to localhost, private IP ranges, and cloud metadata addresses, so to test against a local server, expose it first with a tunneling service like ngrok.
  </Step>

  <Step title="Set a timeout (optional)">
    Set how long Tavly AI waits for your endpoint to respond, in milliseconds. It accepts `1000` (1 second) to `600000` (10 minutes) and defaults to `120000` (2 minutes). If your endpoint doesn't respond in time, the request fails and the agent receives the error message (for example, a timeout error) to act on. By default Tavly AI doesn't retry a custom function, but you can turn on automatic retries with `max_retry` — see [Retries](#retries).
  </Step>

  <Step title="Set request headers (optional)">
    Define custom headers to include with the request. Header values can be static or include [dynamic variables](/build/context/dynamic-variables), such as an auth token you pass in as `{{token}}`.
  ...
  <Step title="Set query parameters (optional)">
    Define query parameters as key/value pairs that Tavly AI appends to the endpoint URL. Values are applied directly and can include [dynamic variables](/build/context/dynamic-variables). Query parameters are never filled in by the LLM — for LLM-supplied values, use the request body parameters below.
  </Step>
  ...
  <Step title="Set response variables (optional)">
    Extract values from the JSON response and save them as [dynamic variables](/build/context/dynamic-variables) to use later in the conversation.
...
When the function is called, Tavly AI sends a request to your endpoint with the following spec.

**Request**

* Headers
  * `X-TavlyAI-Signature`: an HMAC-SHA256 signature of the request body, used to verify the request came from Tavly AI. See [Verify the request is from Tavly AI](#verify-the-request-is-from-Tavly AI) below.
...
* Body (JSON, for POST, PUT, and PATCH only)
  ...
  * `call`: the call object, for context about the call. It includes the transcript up to the moment the request is sent. See Get Call {/* broken-link: /api-references/get-call target page does not exist in this repo */} for the full object.
...
## Verify the request is from Tavly AI

To confirm a request came from Tavly AI, verify the `X-TavlyAI-Signature` header against the raw request body using your Tavly AI API key. For GET and DELETE requests the body is empty, so pass an empty string to the verify function.
...
<CodeGroup>
  ...
  app.post("/check-weather", async (req, res) => {
    ...
    const signature = req.headers["x-tavly-signature"];
    if (
  ...
  async def check_weather(request: Request):
      ...
      valid_signature = tavly.verify(
          ...
          signature=request.headers.get("X-Tavly-Signature"),
      )
...
<Note>You can also restrict your server to Tavly AI's outbound IP address: `100.20.5.228`.</Note>
