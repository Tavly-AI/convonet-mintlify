...
> Run a pre-built function, integration tool, or custom function from a Tavly AI conversation flow function node; it fires on entry and transitions on the result.
...
* [Custom Function](/build/setup/conversation-flow/function-node/custom-function-in-conversation-flow)
* [Integration tools](/build/setup/conversation-flow/function-node/integration-tools-in-conversation-flow) from connected providers like HubSpot, Salesforce, Zendesk, and Cal.com

<Note>
  To check availability or book appointments, use the Cal.com {/* broken-link: /integrations/cal-com-functions target page does not exist in this repo --> or Calendly <!-- broken-link: /integrations/calendly-functions target page does not exist in this repo --> integration tools. The built-in Cal.com functions are no longer offered in the node — see the deprecation notice <!-- broken-link: /deprecation-notice/2026/10-31_legacy_calcom_tools target page does not exist in this repo */} for what happens to flows that still have them.
</Note>
...
* **Fine-tuning Examples**: Can finetune transition. Read more at [Finetune Examples](/build/setup/conversation-flow/advanced/finetune-examples)
