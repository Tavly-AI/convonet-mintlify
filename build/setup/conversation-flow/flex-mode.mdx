---
title : "Flex Mode: dynamic conversation flow execution"
description: "Turn on Flex Mode to compile a Tavly conversation flow into a single structured prompt at runtime, so the agent handles varied user behavior naturally."
---

> Flex Mode compiles a Tavly conversation flow into a single structured prompt at runtime so the agent can handle varied user behavior while keeping flow logic.
