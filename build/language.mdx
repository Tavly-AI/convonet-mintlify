...
> Set a single language for a Tavly agent to control speech recognition, voice pronunciation, and the language the agent uses to respond during calls.
...
For a full per-language breakdown of which voice and speech recognition providers cover each language, see [Language support by provider](/build/language-support). Still picking your stack? See the [TTS provider comparison](/build/speech/tts-provider-comparison) and [speech recognition providers](/build/understanding/asr-providers) for provider-level coverage and trade-offs.
