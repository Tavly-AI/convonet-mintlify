...
---
> Turn on Tavly Agent Handbook presets to add best-practice prompts for personality, accuracy, and safety with one toggle — no manual prompt writing required.
...
<AccordionGroup>
  <Accordion title="Default Tone">
    ...
    * **Professional + Conversational** *(\~910 tokens · Voice only)* — casual and natural, with no robotic assistant vibes: short turns, one question at a time, real recommendations instead of "it depends", and numbers and times spoken the way people say them. Best for sales, front desk, scheduling, and customer success. See [Conversational Mode](/build/response/conversational-mode) for side-by-side examples.
...
<AccordionGroup>
  <Accordion title="Can I customize the content of a preset?">
    No — presets are fixed best-practice prompts. For custom instructions, write them directly in your agent's prompt. See the [Prompt Engineering Guide](/build/response/prompt-engineering-guide) for tips.
  </Accordion>
