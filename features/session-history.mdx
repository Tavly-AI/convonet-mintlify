...
description: "Browse Tavly AI call and chat history in the dashboard: filter sessions, inspect transcripts and analysis, rerun analysis, and export to CSV."
---
...
To investigate several sessions together, click **Select**, check the rows, and open **Conductor**. The selected calls or chats become [context for your question](/conductor/overview#select-calls-and-chats-from-history).
...
<Steps>
  ...
  <Step title="Submit the export">
    Submit the request. Tavly AI generates the CSV in the background. If the request exceeds your workspace's export limit, narrow the filters and try again.
  </Step>
...
<AccordionGroup>
  <Accordion title="How long are calls, chats, and recordings stored?">
    By default, Tavly AI stores session data without automatic deletion. You can set a per-agent [data retention period](/accounts/data-retention) to delete data after 1–730 days. All session history is also permanently deleted if you [delete your account](/accounts/account#delete-your-account).
  </Accordion>
  ...
  <Accordion title="What format are call recordings in?">
    Tavly AI stores call recordings as WAV audio. A call can include a mixed single-channel recording, a recording with each party on a separate channel, and PII-scrubbed versions when configured.
